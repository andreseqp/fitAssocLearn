...
parameters from a data from a reversal learning task. In the experimental set up,
...
is switch. Thus, individuals must reverse their estimate of reward in order to make 
adaptive decisions and choose the rewarding stimulus. In \ref{fig:plotBoussard}, 
...
# dev.off()
# include_graphics("boussard_data.png")
```
...
with a set of preset parameter values. I then evaluate whether the algorithm 
...
The update is performed every trial and is given by the product of the *prediction error*
$(\delta)$ and the *speed of learning* $(\alpha)$. The *prediction error* is 
calcuted as the difference between the reward triggered by
choosing a particular stimulus and the estimation of that reward prior to the
...
any reward reversal. In table \ref{table:param_simple}, I show the values of the
...
#     ggtitle("Repeated reversal vs brainsize")
# dev.off()
include_graphics("simple_DS.png")
...
```{r est_simple,fig.cap='Estimatation of the paramerers used to generate the simulated data. Red points correspond to the real values of the parameteres.'}
gc()
...
# dev.off()
include_graphics("simple_mcmc_intervals.png")
...
# 
# rm(list=c("preds","preds_df","preds_long"))
# predictions %>% mutate(treatment=as.factor(treatment)) %>% 
#   ggplot(aes(y=success,x=Trial,col=treatment))+
#     stat_summary(fun = mean,geom = "point")+
#     stat_summary(fun = mean,geom = "line")+
...
#   facet_grid(~treatment)
# dev.off()
include_graphics("simple_ppchecks.png")
```
...
#     ggtitle("Repeated reversal vs brainsize")
# dev.off()
include_graphics("reversal_data.png")
```
...
#   geom_point(data=realVals,aes(x=values,y=parameters),colour='red',size=2)
# dev.off()
include_graphics("reversal_mcmc_interv.png")
...
#   facet_grid(~treatment)
# dev.off()
include_graphics("reversal_ppchecks.png")
```

## Let' fit a model with different learning rates in reversal blocks to the Boussard *et al* data set
...
# mcmc_intervals(posteriors)
# dev.off()
include_graphics("boussard_mcmc_interv.png")
...
#     facet_grid(brainsize~reversal)
# dev.off()
include_graphics("boussard_ppchecks.png")
...
dev.off()
include_graphics("boussard_mcmc_interv_rev.png")
# rm(list=c("pars2plot","posteriors"))
...
mean_ind <-mean_ind %>%
  mutate(reversal= as.integer((Ttrial-1)/30)) %>%
  mutate(RTrial=((trial-1) %% 30)+1)
