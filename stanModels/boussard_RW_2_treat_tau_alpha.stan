...
transformed parameters {
  ...
  vector <lower=0,upper=1>[N] alphaID_t;
  {
    real tmpTauUn = 0;
    int countTreat;
    for (i in 1:N) {
      tmpTauUn += mus[1]+ sigmas[1]*tauID[i];
      countTreat = 0;
      alphaID_t[i] = inv_logit(mus[2]+sigmas[2]*alphaID[i]);
      for (j in 1:B){
        tmpTauUn += tauT[countTreat+treat_ID[j,i]];
        countTreat += BE[j];
      }
      tauID_t[i]  = exp(tmpTauUn);
      tmpTauUn = 0;
    }
  }
}
    ...
    for (tr in 1:Tr){
      ...
      if(block_r[tr,2]==1) {
        ...
        if(y_pred[ind,tr]==1){
          ...
          est_values_p[2] += alphaID_t[ind]*pred_error_p;
          // update action values
        }
        else{
          pred_error_p =  block_r[tr,1]- est_values_p[1];
          est_values_p[1] += alphaID_t[ind]*pred_error_p;
          // update action values
        }
      }
      else {
